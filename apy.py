...
import warnings
import os  # Añadido para gestionar las rutas de almacenamiento de forma limpia
...
def cargar_sistema_ia():
    try:
        # Definimos la ruta de la carpeta interna 'data/modelos'
        carpeta_modelos = os.path.join("data", "modelos")
        
        # Construimos las rutas completas para cada archivo binario (.pkl)
        ruta_modelo = os.path.join(carpeta_modelos, "clasificador_tipos_smote.pkl")
        ruta_scaler = os.path.join(carpeta_modelos, "escalador_features.pkl")
        
        # Cargamos los modelos predictivos desde la nueva ubicación
        modelo = joblib.load(ruta_modelo)
        scaler = joblib.load(ruta_scaler)
        return modelo, scaler
    except Exception as e:
        # En caso de error (ej. si la carpeta o los archivos no existen aún)
        return None, None
